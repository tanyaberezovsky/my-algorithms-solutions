...
func minimumBribes(q: [Int]) -> Void {
   // var d = [Int:Int]()
    ...
    while i < q.count {
        print(q)
        let expectedVal = i+1
        //print("i \(i) val \(val) q[i] \(q[i])")
        
        if q[i] == expectedVal {
            i += 1
            continue
        }
        if q[i] > expectedVal + 2 || q[i] < expectedVal - 2 {
            print("Too chaotic")
            return
        } else {
            if i + 1 >= q.count {
...
//                return
            ...
            q.swapAt(i, i + 1)
            bribes += 1
//            let insertIndex = q[i] - expectedVal
//
//            diff = diff + insertIndex
//
//            q.insert(q[i], at: i + insertIndex + 1)
//
//            q.remove(at: i)
            //print(q)
        }
...
//minimumBribes(q: [1, 2, 5, 3, 6, 7, 8, 4])
